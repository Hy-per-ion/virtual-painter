...
class handDetector:
    ...
    def fingersUp(self):
        if len(self.lmList) != 0:
            fingers = []
            # Thumb
            if self.lmList[self.tipIds[2]][1] < self.lmList[self.tipIds[3]][1]:
                # Left thumb
...
def main():
    ...
    while True:
        ...
        img = detector.findHands(img)  # add draw = false to remove the lines in the hand
        lmList = detector.findPosition(img)  # add draw = false to remove the circle drawn at all the ids
        if len(lmList) != 0:
            print(lmList[0])
